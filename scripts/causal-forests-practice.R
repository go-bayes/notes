...
# 
# Is there a recommended approach for dealing with missing Y values in grf? 
# 
# Below is a simplified example highlighting my issue, and proposed workaround. 
# 
# Thank you for your assistance and for developing this really great sofware! 
# 
# Joe
...
# problem: loss-to-follow-up leads to missingness at time2
...
#  1:  fill out the exampl by simulating survey weights 
survey_weights <- runif(nrow(X), min = 0.5, max = 2)

#  2: create binary indicator for missing Y2 values
Y2_missing <- as.integer(is.na(Y2))

#  3: fit a censoring model
censoring_model <- glm(Y2_missing ~ ., data = as.data.frame(cbind(X, W1)), family = "binomial")
censoring_model
#  4: compute inverse probability of censoring weights
uncensored_probs <- predict(censoring_model, type = "response", newdata = data.frame(X, W1))
